...
**Confirmed direction (2026-09-03, user-directed): a provider fallback chain, not a single-provider pick.**
StageEgo already proves this exact pattern in `characterFetch.ts` (try source A → B → C, normalize results into one shape) — reusing that architecture for avatar generation is lower-risk than introducing a new pattern. Planned chain, richest/most-capable first:
1. **Gemini 2.5 Flash Image** — primary. Best fit, reuses existing BYOK key infra.
2. **Cloudflare Workers AI** — secondary. Different free-tier bucket entirely, so hitting Gemini's daily request cap doesn't just fail outright — it overflows here instead.
3. **Canvas-filter fallback (`avatarFilters.ts`, already built)** — tertiary/always-available. Fires if both API tiers are exhausted, rate-limited, or the user has no API key configured at all. Zero-cost, zero-dependency, already shipped.
- [ ] Design the normalized result shape (mirroring `CharacterInfo` in characterFetch.ts) so callers don't care which tier answered
- [ ] Design the try-chain function itself (mirroring the source-priority pattern already in characterFetch.ts)
- [ ] Confirm Cloudflare Workers AI's actual request format/auth model before wiring it in — not yet verified hands-on, only confirmed via research

- [x] **Zero-cost emotion-variant fallback tier** — DONE 2026-09-03. `avatarFilters.ts`: `generateEmotionVariant()`/`generateAllEmotionVariants()` produce tinted per-emotion variants of one base portrait via canvas `ctx.filter` (brightness/saturate/hue-rotate/contrast), no external API, no cost. Wired into CharacterSelect.tsx's existing per-emotion art upload UI as a "✨ Fill gaps from base photo" button — fills only empty emotion slots, never overwrites a manually-uploaded one. Honest limit stated in both code comments and the button's UI tooltip: this tints/tones only, it cannot redraw facial expression/pose. Not a replacement for true AI-generated multi-frame art — a same-day-usable baseline tier underneath it. Now also serves as the confirmed tier 3 of the fallback chain above.
...
- [x] Prototype Cloudflare Workers AI avatar generation (chain tier 2) — WRITTEN 2026-09-03, `avatarGenerate.ts`: `generateAvatarViaCloudflare()`. **Not yet live-tested** (same status as Gemini tier 1 — no network path or credentials in this sandbox). **New finding, genuinely important:** Cloudflare authenticates with TWO separate credentials (Account ID + API Token), unlike Gemini/Anthropic/OpenAI's single-key pattern — StageEgo's Settings UI only has one API Key field shared across providers. This is a real open decision, not resolved: (a) add a second Cloudflare-only Settings field, (b) accept a combined "accountId:token" paste split client-side, or (c) drop Cloudflare from the near-term chain and ship Gemini + canvas-filter as a 2-tier chain instead. Needs a decision before this tier is wired into any UI — flagged prominently in the file header too.
- [ ] Build the actual fallback-chain orchestration function once tiers 1 & 2 are individually prototyped and confirmed working
