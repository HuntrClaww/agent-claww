...
| 13 | Gemini key provided 2026-09-11 not yet tested | **PENDING — blocked by sandbox network** | N/A | `generativelanguage.googleapis.com` is not in this sandbox's egress allowlist, so the key Arthur pasted couldn't be curl-tested directly from this environment. Also note: the key's format (`AQ.Ab8RN6...`) doesn't match the usual AI-Studio/Gemini key pattern (`AIzaSy...`) — worth Arthur double-checking it was generated from the right Google Cloud console/product before relying on it. Needs either sandbox network settings updated, or Arthur testing it himself locally (npm run dev + Settings > Standard Assistant > Test API Connection, which will now also populate the new Performance Log tab). Do not paste real API keys into this file or into committed code — this row intentionally omits the actual key value. |
