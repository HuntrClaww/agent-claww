...
| 52 | **AUDIT (2026-09-14, Arthur-requested):** cross-checked every claimed-done UI feature in Phases 1-8.5 against the live source tree | See note below | Arthur asked for a full pass: verify every feature discussed across HANDOVER/CORE_VISION/VISUAL_NOVEL_UI_SPEC actually exists and is wired into the UI, deferred items excused, but anything said to be "placed in" that isn't visible needs fixing. Grepped the actual source for the two-sided coin UI, Lore-Locked/Open-World labels, portrait upload + per-emotion slots, theme color picker, fork UI, all 5 Settings tabs, temperature slider, Stop button, Voice Studio, Help Hub, character search modal, and the avatar "Fill gaps" button - all confirmed present and wired, not just documented. One real miss found and fixed (see Known Issue #19 below, now HANDLED). One real gap found and NOT fixed - logged as Known Issue #20, needs Arthur's input before touching it. Also cross-referenced VISUAL_NOVEL_UI_SPEC.md's "Advanced Theme System" (preset color schemes + a `ThemeSettings`/custom-colors interface, ~line 388) against the source: confirmed zero implementation anywhere, and HANDOVER never claimed otherwise - this was always spec-only, not a regression. Directly relevant to the pending color-picker work in Section 10/Known Issue #18: the spec already has concrete preset palettes and a CSS-variable approach worth reusing rather than designing from scratch. |
...
| 14 | `avatarGenerate.ts` image model (`gemini-2.5-flash-image`) shuts down Oct 2, 2026 | **✅ FIXED 2026-09-14 (commit 998442b)** | avatarGenerate.ts | Found while researching issue #12 (2026-09-12), fixed during the bug-hunting pass on 2026-09-14. Switched to `gemini-3.1-flash-image-preview`, Google's own named replacement per their deprecations page. Still a preview model, not a stable alias — worth re-checking if it ever 404s. |
...
| 18 | **MAJOR:** Full UI/UX redesign requested | **IN PROGRESS as of 2026-09-14 — 2 commits pushed (a464869 cutout/avatar; 912389e accent theme system), both now visually verified via Playwright (see Section 8) after fixing Known Issue #21** | Everything visual | See Section 10 for full detail and the resolved design direction. Arthur directed the new session to skip the structural/navigational plan for now and start on visual style/theme directly — that plan is deferred, not cancelled. Screenshots taken 2026-09-14 confirm: the cutout portrait genuinely floats with no card/box (after the #21 fix), the header ID avatar renders correctly on both desktop and mobile, the sidebar glow applies, and the Appearance tab's presets actually change the send button / message border live after Save. Arthur has not yet looked at it himself — still worth his own pass, but this is stronger confirmation than "typecheck and build pass" alone. Remaining open: background image selection, and the larger layout-level theming (chat style/portrait position/message density) from VN_SPEC's Advanced Theme System that the accent-color commit deliberately left untouched. |
| 19 | Stale "AI Performance Coach" branding survived in `Auth.tsx` | **✅ FIXED 2026-09-14 (commit 2a5a6d0)** | Auth.tsx | Known Issue #48 (2026-09-10) fixed this branding in index.html/package.json/App.tsx but never checked Auth.tsx - the login/guest landing screen, the very first thing anyone sees. Still had "Your AI Performance Coach" and "Master your presence. Refine your delivery. Elevate your performance." in two spots. Replaced with copy matching the already-corrected positioning (two-sided character chatbot). Confirmed via grep this was the last remaining "coach" reference anywhere in src/, index.html, package.json, or README.md. |
| 20 | **NOT FIXED, needs Arthur's decision:** "Continue with Google" / "Continue with GitHub" on the Auth screen do nothing real | **OPEN — found 2026-09-14** | Auth.tsx, App.tsx | Both buttons call the exact same `onLogin` prop, which just does `setSessionState('loggedIn')` in App.tsx - no OAuth redirect, no Supabase call, no distinction between "Google," "GitHub," or each other. Functionally identical to Guest mode except the `isGuest` flag flips to false. `lib/supabase.ts` already has a real, working Supabase client wrapper (`isSupabaseConfigured()`, env-var gated) sitting completely unused - nothing in the app imports it. This isn't a regression (Phase 9 always said backend auth was deferred pending a working Supabase auth flow), but the buttons actively look real and functional, which is a different, more specific problem than "auth isn't built yet." Two honest paths, Arthur's call: (a) relabel as "Coming soon"/disable until real OAuth is wired, or (b) actually wire Supabase OAuth now (needs Arthur to register Google/GitHub OAuth apps + set redirect URIs + provide `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` - real setup work outside what a session can do blind). Left untouched pending that decision. |
| 21 | **CRITICAL:** portrait uploads lost transparency — the VN cutout redesign never actually worked | **✅ FIXED 2026-09-14 (commit df27f1a)** | imageCompress.ts | `compressPortrait()` always exported via `canvas.toDataURL('image/jpeg', ...)` — JPEG has no alpha channel, so any transparent-background portrait was silently flattened to a solid black background before `CharacterPortrait.tsx` (Known Issue #18's cutout redesign) ever saw it. The CSS side was correct the whole time. **Found by actually running the app with Playwright and uploading a real transparent PNG — see the note in Section 8 above.** Confirmed fixed the same way: re-ran the identical flow, saved `portraitUrl` is now `data:image/png` (was `data:image/jpeg`), screenshot shows the figure floating with no box. Fix detects transparency and exports PNG (dimension-shrink to fit the size cap, since PNG has no quality knob) only when needed; ordinary opaque photos keep the better-compressing JPEG path unchanged. |
| 22 | Animated background layer painted nothing at all | **✅ FIXED 2026-09-14 (commit 1645356)** | index.html, index.css, appearance.ts | `#bg-layer` (position:fixed, z-index:-1) rendered completely invisible despite every computed style being correct — verified via `getComputedStyle`, which showed the right gradients, opacity and animation. Cause: index.html's inline pre-React shell styles set `background` on `html`, which stops `body`'s background from propagating to the canvas; `body` then paints its own background as an ordinary block background, and in CSS paint order block backgrounds come AFTER negative-z-index elements — so it covered the layer entirely. Fix: base color lives on `html`, `body` is transparent. **Only found by sampling actual rendered pixels** (all exactly `#0f172a` before the fix) — code review and computed styles both looked fine. Worth remembering: "the CSS is correct" and "the pixels are correct" are different claims. |
| 23 | Light mode was unreadable (light-on-light text) | **✅ FIXED 2026-09-14 (commit 1645356)** | index.css | Every component was written dark-first with hardcoded Tailwind slate classes (`text-slate-100`, `bg-slate-800/60`, `border-slate-700`), so enabling light mode produced pale text on pale panels across the whole app. Fixed by remapping the slate scale under `html.theme-light` rather than refactoring every component in one pass — the scale is effectively "distance from the background", so inverting it preserves each element's intended hierarchy. **This is a documented bridge, not the end state:** new components should use `--text-primary` / `--surface-*` / `--glass-*` directly. Anything added to a component with a hardcoded slate class will need its own remap line, so prefer the vars. OLED got a smaller version of the same treatment (mid-slate panels read as obvious grey rectangles against true black). |
| 24 | Voice & Speech settings (built same day) weren't wired to real behavior; one slider had no real API to back at all | **✅ FIXED 2026-09-14 (commit 1b7dc61)** | ChatWindow.tsx, voiceEngine.ts, voicePrefs.ts, SettingsModal.tsx | Caught while doing the Voice Studio pass, not by Arthur - worth internalizing the pattern: **a Settings toggle that saves to localStorage but that no code ever reads is functionally the same bug as the JPEG-transparency issue (#21) or the invisible background layer (#22)** - it looks done, isn't. Specifically: (1) 7 of 8 global voice prefs were never consulted anywhere near speakExpressive() - now merged in as fallbacks under the character's own settings. (2) "Auto-speak" duplicated the already-working voiceModeOn/voice_mode_on mechanism instead of using it - now points at the same key. (3) A "silence wait" slider had nothing real to control - the Web Speech API has no standard silence-duration property; removed rather than ship a fake setting. **Process note for future sessions: when adding a Settings control, grep for where the value is actually READ before considering the feature done, not just where it's saved. Saving is not the same as working.** |
| 25 | IndexedDB indexes silently drop records with a `null`/`undefined` indexed field - `listVoiceProfiles(null)` returned empty even for genuinely-saved profiles | **✅ FIXED 2026-09-14 (commit 0a67122)** | voiceStorage.ts | `null` and `undefined` are not valid IndexedDB key types, so a record saved with `characterId: null` (every Voice Lab profile made during character creation, before the character has a real id) was silently excluded from the `characterId` index at write time - not an error, just absent from that index. The write itself always succeeded; direct object-store inspection confirmed the record was there. But `listVoiceProfiles()` queried the INDEX for key `null`, which always came back empty regardless. **Only found by actually closing and reopening Voice Lab in a real browser and checking IndexedDB at each step** - code review alone would not have caught this, since every individual piece (the write, the index definition, the query) looks correct in isolation. Fixed by falling back to a full scan + in-memory filter whenever the id isn't a real, truthy character id; the index is still used for the normal case of a specific character's own profiles. **Process note: this is the same shape as Known Issue #24 (a setting that saves but isn't read) one level deeper - here the data WAS read, just from the wrong place (an index that silently excludes the exact records being asked for). When a "should be there" value comes back empty, check whether an index/query path could be excluding it before assuming the write failed.** |
...
- **localStorage keys in use:** `user_api_key`, `profanity_filter`, `ai_temperature`, `reduce_visual_effects`, `theme_preference`, `stageego_characters`, `app_theme_choice`, `app_theme_custom_primary`, `app_theme_custom_secondary` (accent theme, added 2026-09-14 — see `lib/themePresets.ts`).
- **Events in use:** `profileUpdated` (re-init API client, reapply dark/light + accent theme), `charactersUpdated` (refresh saved list).
...
- **⚠️ IMPORTANT, discovered 2026-09-14 — this sandbox CAN actually render and screenshot the app.** Earlier sessions (including this one, initially) told Arthur "I can't render/screenshot this environment" — that was wrong, not a real limitation. Playwright (Python, v1.56.0) plus a Chromium binary are pre-installed (`/opt/pw-browsers`, env var `PLAYWRIGHT_BROWSERS_PATH`). Pattern that works: start the dev server AND run the Playwright script in the SAME bash command (`(nohup npm run dev -- --host 0.0.0.0 --port 5173 > /tmp/vite.log 2>&1 &) && sleep 5 && python3 script.py`) — a background server started in one command is gone by the next command, since each tool call is a fresh shell. Use `page.screenshot(path=..., full_page=True)` then the `view` tool on the resulting PNG to actually look at it. This is how Known Issue #21 below was found — a real bug that pure code review missed entirely. **Use this before telling Arthur something "looks right" or "should work" — actually look first.**
...
- 2026-09-14: First redesign commit made locally (typecheck + build
  ...
  mobile and desktop headers. **PUSHED 2026-09-14** (commit a464869;
  docs/HANDOVER update in 8c953e1) — verified local HEAD == origin/main
  after push. Not yet visually confirmed by Arthur locally — per Rule 3,
  "complete" needs pushed AND working, so treat this as shippped-but-
  unverified until he says it looks right after pulling.
- 2026-09-14: **appearance engine shipped and pushed** (commit 1645356).
  `lib/appearance.ts` replaced `lib/themePresets.ts` (deleted) as the
  single owner of every visual setting. Adds: Light/Dark/OLED modes,
  animated Aurora/Gradient/Solid backgrounds tinted by the accent,
  saturation + contrast sliders, separate Blur and Sheen glass
  controls, and four motion styles. Live preview applies to the real
  document while the modal is open; Cancel reverts. The glass material
  now reads from the vars and has a reflective hover sweep.
  Two real bugs fixed in the process, both caught by screenshots
  rather than review — see Known Issues #22 and #23.
  **Still open from Arthur's original ask:** user background *images*
...
- [x] **Voice Studio** — modernized (real icons, glass styling) and,
      more importantly, made real: 7 of 8 global Voice & Speech
      settings were saving to localStorage but nothing read them.
      Added Volume + Expressiveness as genuine per-character params.
      See Known Issue #24 for the full story — worth a read before
      adding the next Settings control.
...
- Still not started: the user-selectable color/theme picker,
  ...
  localStorage-only).
- 2026-09-14 (later same day): color/theme picker built and **PUSHED**
  (commit 912389e) — new `lib/themePresets.ts` (4 presets from
  VISUAL_NOVEL_UI_SPEC.md's Advanced Theme System + a custom option
  using two native `<input type="color">` spectrum pickers), a new
  Settings > Appearance tab, applied as `--user-accent` /
  `--user-accent-secondary` CSS vars. Wired into user message bubbles,
  the AI message border, the send button, and the sidebar glow, each
  falling back through character-color → user-theme → hardcoded teal.
  Scoped to accent colors only — background/layout theming (also in
  VN_SPEC's Advanced Theme System, e.g. `chatStyle`/`portraitPosition`/
  `messageLayout`) deliberately NOT touched: backgrounds are hardcoded
  Tailwind classes across dozens of components with no variable to
  hook into yet, and this sandbox can't render/screenshot to catch a
  contrast mistake, so that's a separate, more careful pass. Background
  image selection is still the next open item from Arthur's original
  ask, not yet started. Not yet visually confirmed by Arthur.
